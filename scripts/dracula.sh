...
main() {
  ...
  *)
  ...
  true)
    ...
    current_flags="#{?window_flags,#[fg=${gray}]#{window_flags},}"
    ;;
  esac
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${cyan},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}],} ${left_icon} #[fg=${cyan},bg=${gray}]#{?client_prefix,#[fg=${orange}],}${left_sep}"
    powerbg=${gray}
  else
    tmux set-option -g status-left "#[bg=${cyan},fg=${dark_gray}]#{?client_prefix,#[bg=${orange}],} ${left_icon}"
  fi
